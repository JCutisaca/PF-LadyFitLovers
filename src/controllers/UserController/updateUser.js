...
const updateUser = async ({ id, name, surname, email, phone, password,newPassword, address, typeUser, userBan, image }) => {
    ...
    const updatePassword = password ? (await bcrypt.compare(password, findUser.dataValues.password) ? newPassword : findUser.password) : findUser.password;
    const update = await User.update({
        name: name ? name : findUser.name,
        surname: surname ? surname : findUser.surname,
        email: email ? email : findUser.email,
        phone: phone ? phone : findUser.phone,
        password: newPassword ? await bcrypt.hash(updatePassword, 10) : findUser.password,
